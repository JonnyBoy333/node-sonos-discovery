...
function applyPreset(preset) {
  ...
  if (preset.nightMode) {
    promise = promise.then(() => coordinator.nightMode(preset.nightMode)
      .catch(err => logger.warn(err, 'Night Mode failed')));
  }

  if (preset.speechEnhancement) {
    promise = promise.then(() => coordinator.speechEnhancement(preset.speechEnhancement)
      .catch(err => logger.warn(err, 'Speech Enhancement failed')));
  }

  if (preset.subCrossover) {
    promise = promise.then(() => coordinator.subCrossover(preset.subCrossover)
      .catch(err => logger.warn(err, 'Sub Crossover failed')));
  }

  if (preset.subDisable) {
    promise = promise.then(() => coordinator.subDisable(preset.subDisable)
      .catch(err => logger.warn(err, 'Sub Disable failed')));
  }

  if (preset.subEnable) {
    promise = promise.then(() => coordinator.subEnable(preset.subEnable)
      .catch(err => logger.warn(err, 'Sub Enable failed')));
  }

  if (preset.subGain) {
    promise = promise.then(() => coordinator.subGain(preset.subGain)
      .catch(err => logger.warn(err, 'Sub Gain failed')));
  }

  if (preset.subPolarity) {
    promise = promise.then(() => coordinator.subPolarity(preset.subPolarity)
      .catch(err => logger.warn(err, 'Sub Polarity failed')));
  }
